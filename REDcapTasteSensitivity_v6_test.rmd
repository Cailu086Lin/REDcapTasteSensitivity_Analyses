...
mydirectory<-"C:/Users/clin/Dropbox/REDCap/3.Resubmit/data/dat"
...
# Load  library
pacman::p_load(readxl, plyr, dplyr, mice, knitr, magrittr, tableone, reshape, kableExtra, broom, stringr, rgenoud, tidyverse, Hmisc, uwIntroStats, ggpubr, epitools, survey, lubridate, HardyWeinberg, stringr, data.table, effectsize, lsr, MBESS)
```
...
d1 <- mutate(d1, dichosmoking = ifelse(dichosmoking == "4", "0", "1"), NeverSmoker = ifelse(NeverSmoker == "4", "1", "0"))
...
#Remove missing values for mydata
mydata<-redcap#%>%drop_na(sex,age,dichosmoking)
test_before <- unique(redcap$tissueid) %>% as.data.frame
test_after <- unique(mydata$tissueid) %>% as.data.frame #1130 patients dropped

#Immunodeficiencies
mydata$immunodef <- rowSums(as.data.frame(lapply(mydata[c("wegeners","churgstrauss","sarcoidosis","sjogrens","autoimmuneother","HIV","transplant","immunodefother")],as.numeric)))

#SNOT-22
mydata$snot22 <- rowSums(mydata[c("need_to_blow_nose","sneezing","runny_nose","nasal_obstruction","loss_of_smell_or_taste","cough","post_nasal_discharge","thick_nasal_discharge","ear_fullness","dizziness","ear_pain","facial_pain_pressure","difficulty","wake_up_at_night","lack","wake_up_tired","fatigue","reduced_productivity","reduced_concentration","frustrated","sad","embarrassed")])

#CT scores
mydata$ctscores <- rowSums(as.data.frame(lapply(mydata[c("right_sphenoid","right_post_ethmoid","right_ant_ethmoid","right_o_complex","right_maxillary","right_frontal","left_sphenoid","left_post_ethmoid","left_ant_ethmoid","left_o_complex","left_maxillary","left_frontal")],as.numeric)))

#Haplotypes
#grep("Haplotype", colnames(mydata))
grep("catgeno", colnames(mydata))
haplotype_before <-
      ddply(mydata,~Haplotype,summarise,number_of_pts=length(unique(tissueid)))
mydata$Haplotype[which(mydata$Haplotype != "AVI/AVI" &
                        mydata$Haplotype != "AVI/PAV" & 
                        mydata$Haplotype != "PAV/PAV")] <- "Other"
haplotype_after <-
      ddply(mydata,~Haplotype,summarise,number_of_pts=length(unique(tissueid)))
haplotype_after

mydata$TAS2R38 <- mydata$Haplotype

#Other numeric variables
mydata$age <- as.numeric(mydata$age)
mydata$bmi <- as.numeric(mydata$bmi)
mydata<-rename(mydata, c(dichosmoking="EverSmoker"))

#Prep for combo scores
names(mydata)

pDB <- grepl("DBIntensity", names(mydata), fixed = TRUE)
pSUC <- grepl("SucroseIntensity", names(mydata), fixed = TRUE) 
pQUI <- grepl("QuinineIntensity", names(mydata), fixed = TRUE) 
pPTC <- grepl("PTCIntensity", names(mydata), fixed = TRUE) 
pNaCl <- grepl("NaClIntensity", names(mydata), fixed = TRUE)

mydata <- data.frame(mydata,DB=(rowSums(mydata[, pDB]))/2,
                 SUC=(rowSums(mydata[, pSUC]))/2,
                 QUI=(rowSums(mydata[, pQUI]))/2,
                 PTC=(rowSums(mydata[, pPTC]))/2,
                 NaCl=(rowSums(mydata[, pNaCl]))/2)

mydata$water <- (mydata$MilliQWaterIntensity1 + mydata$MiliQH2OIntensity9)/2

names(mydata)

mydata$All_total <- mydata$DB + mydata$SUC + mydata$PTC + mydata$QUI + mydata$NaCl + mydata$water

mydata$SUC_prop <- 100-(mydata$SUC/mydata$All_total*100)
mydata$DB_prop <- mydata$DB/mydata$All_total*100
mydata$QUI_prop <- mydata$QUI/mydata$All_total*100
mydata$PTC_prop <- mydata$PTC/mydata$All_total*100
mydata$NaCl_prop <- mydata$NaCl/mydata$All_total*100
mydata$water_prop <- mydata$water/mydata$All_total*100

mydata$DB_combo_prop <- mydata$DB_prop+mydata$SUC_prop
mydata$QUI_combo_prop <- mydata$QUI_prop+mydata$SUC_prop
mydata$PTC_combo_prop <- mydata$PTC_prop+mydata$SUC_prop

#To improve matching, categorize continuous variables
quantile(mydata$age, na.rm=T)
mydata$agecat<-cut(as.numeric(mydata$age), c(18,34,48,59,83))
mydata$agecat2<-as.character(mydata$agecat)


#Clean up categorical variables
mydata$sex<-factor(mydata$sex)
mydata$EverSmoker<-factor(mydata$EverSmoker)
mydata$asthma1<-mydata$asthma
mydata$asthma1<-as.numeric(mydata$asthma1)
mydata$asthma<-mydata$asthma1
mydata$asthma1<-NULL

#Create the CRSsNP noAsthma variable
mydata$sNPsAsthma<- ifelse(mydata$polyps==0 & mydata$asthma==0,1,0)

#mydata needs your group variable to be LOGICAL, and not just dichotomous
mydata$control<-as.logical(mydata$control)

#Create column catgeno
mydata$catgeno <- mydata$TAS2R38 

#Create a DF just for the mydata call
final <- mydata
mydatadf <-final %>%
...
mydatadf <- mydatadf[which(is.na(mydatadf$Haplotype)==F),] 


mydatadf$aerd[is.na(mydatadf$aerd)]<-""
mydatadf$prevfess[is.na(mydatadf$prevfess)]<-""
...
table1<-CreateTableOne(vars=baseline, data=mydatadf, factorVars=catVars, strata=c("control"))
table1<-print(table1, exact=catVars, smd= TRUE)

table2<-CreateTableOne(vars=taste, data=mydatadf, strata=c("control"))
table2<-print(table2,smd= TRUE)

mydatadf_cases<-mydatadf[which(mydatadf$control==FALSE),]
mydatadf_cases$ars[is.na(mydatadf_cases$ars)]<-"0"
tablecomicro<-CreateTableOne(vars=basecomicro, data=mydatadf_cases, factorVars=catVars)
...
```
CRS=Chronic rhinosinusitis; P=p-value; SMD=standardized mean differences; N=number of subjects; %=percent; White = people of European ancestry by self-report, Non-white = otherwise; yrs=years of age; SNOT-22, Sinonasal Outcome Test, scores range from 0 to 110 with higher values indicating worse symptoms. TAS2R38 diplotype, the combination of two haplotypes of the bitter receptor, PAV is the 'taster' form whereas the AVI is the 'on-taster' form. 
...
haplo<-ddply(mydatadf,control~Haplotype,summarise,Number=length(unique(tissueid)))
...
### (1) GLM with Covariate Analysis (CRS vs Controls)
Looking at age, sex, smoking history and comorbidity asthma cordifferences between the sinusitis and control patient cohorts and effect on taste test scores for PTC, DB, QUI, and SUC.

```{r covariate, include=FALSE, echo=FALSE}

mydata1<-mydatadf[c("tissueid","PTC", "DB", "QUI", "SUC", "NaCl", "water", "DB_combo_prop", "QUI_combo_prop", "PTC_combo_prop", "snot22","control", "age", "sex", "TAS2R38", "EverSmoker" , "asthma")]

#loop
results_lm <- data.frame()
results_lmphc <- data.frame()
varlist <- names(mydata1)[2:11]
for(var in varlist){
  # dynamically generate formula
  fmla <- as.formula(paste0(var, "~control+age+sex+EverSmoker+asthma"))
   #fit glm model
  fit <- aov(lm(fmla, mydata1))
  summaryw <- summary(lm(fmla, mydata1))
  fdendf.w <- summaryw$fstatistic[[3]]
  results <- data.frame(tidy(fit),fdendf.w)
  beta<-data.frame(tidy(lm(fmla, mydata1))[2:6,2:5])
  beta[nrow(beta)+1,]<-NA
  results<-bind_cols(results, beta)
  results<-bind_cols(results, eta_squared(fit))
...
results_lm$Compound<-c("PTC","PTC","PTC","PTC","PTC","PTC", "DB","DB","DB","DB","DB","DB",  "Quinine","Quinine","Quinine","Quinine","Quinine","Quinine", "Sucrose","Sucrose","Sucrose","Sucrose","Sucrose","Sucrose","NaCl", "NaCl","NaCl","NaCl","NaCl","NaCl","H2O", "H2O","H2O","H2O","H2O","H2O","DB.Combo","DB.Combo","DB.Combo","DB.Combo","DB.Combo","DB.Combo", "QU.Combo","QU.Combo","QU.Combo","QU.Combo","QU.Combo","QU.Combo","PTC.Combo","PTC.Combo","PTC.Combo","PTC.Combo","PTC.Combo","PTC.Combo","SNOT-22","SNOT-22","SNOT-22","SNOT-22","SNOT-22","SNOT-22")
...
results_lm<-results_lm[,c(14,1:11,13)]
results_lm %>%
  format_model_table %>%
  kable (digits=1, caption="GLM CRS result", col.names=c("Compound","term","df","sumsq","meansq","F","p","dendf","beta", "std.err","statistic",  "p.value", "Eta_Sq")) %>%
  kable_styling(bootstrap_options = c("hover", "condensed"), full_width = F)%>%
  row_spec(c(7,13,19,25,31), bold = T, color = "white", background = "#D7261E") %>%
...
### (2) Stratifying by Polyp Status (option 1)

```{r covariate2, include=FALSE, echo=FALSE}

mydata1<-mydatadf[c("tissueid","PTC", "DB", "QUI", "SUC", "NaCl", "water", "DB_combo_prop", "QUI_combo_prop", "PTC_combo_prop", "snot22","control", "age", "sex", "TAS2R38", "EverSmoker" , "asthma", "polyps")]
mydata1$control=ifelse(mydata1$control=="TRUE", "CNTR", ifelse(mydata1$control=="FALSE" & mydata1$polyps==0, "CRSsNP", "CRSwNP"))
...
results_lm %>%
  format_model_table %>%
  kable (digits=1, caption="GLM CRSsNP+CRSwNP result", col.names=c("Compound","term","df","sumsq","meansq","F","p","dendf","beta", "std.err","statistic",  "p.value", "Eta_Sq")) %>%
  kable_styling(bootstrap_options = c("hover", "condensed"), full_width = F)%>%
  row_spec(c(7,13,19,25,31), bold = T, color = "white", background = "#D7261E") %>%
  row_spec(c(1,26), bold = T, color = "grey", background = "#E0E0E0")
```
...
```{r echo=FALSE, warning=FALSE}
results_lmphc<-results_lmphc[,c(8, 1:7)]
kable(results_lmphc , caption="GLM CRSsNP+CRSwNP, Posthoc test for case_control") %>%
  kable_styling(bootstrap_options = c("hover", "condensed"), full_width = F)

```


## (2) Stratifying by Polyp Status (option 2)
...
d.polyps <-
      ddply(mydatadf,control~polyps,summarise,number_of_pts=length(unique(tissueid)))
d.polyps

d.CRSsNP<-mydatadf[which(mydatadf$polyps=="0" & mydatadf$control==0),] #cases without polyps
d.CRSwNP<-mydatadf[which(mydatadf$polyps=="1" & mydatadf$control==0),] #cases with polyps
d.Controls <-mydatadf[which(mydatadf$control==1),] #controls
d.AllsNP<-mydatadf[which(mydatadf$polyps=="0"),] #all no polyps
...
d.AllsNP1<-na.omit(d.AllsNP[c("tissueid","PTC", "DB", "QUI", "SUC", "NaCl", "water", "DB_combo_prop", "QUI_combo_prop", "PTC_combo_prop", "snot22","control", "age", "sex", "TAS2R38", "EverSmoker", "asthma")])
#loop
...
for(var in varlist){
  ...
  fdendf.w <- summaryw$fstatistic[[3]]
  results <- data.frame(tidy(fit),fdendf.w)
  beta<-data.frame(tidy(lm(fmla, d.AllsNP))[2:6,2:5])
  beta[nrow(beta)+1,]<-NA
  results<-bind_cols(results, beta)
  results<-bind_cols(results, eta_squared(fit))
  ...
  leastsquarea = lsmeans(fit,
                      pairwise ~ control,
                      adjust="tukey", data=d.AllsNP)
  phc<-tidy(leastsquarea$contrasts)
  results_lm_sNPphc<-rbind(results_lm_sNPphc, phc)
}

results_lm_sNP$Compound<-c("PTC","PTC","PTC","PTC","PTC","PTC", "DB","DB","DB","DB","DB","DB","Quinine","Quinine","Quinine","Quinine","Quinine","Quinine", "Sucrose","Sucrose","Sucrose","Sucrose","Sucrose","Sucrose","NaCl", "NaCl","NaCl","NaCl","NaCl","NaCl","H2O","H2O","H2O","H2O","H2O","H2O","DB.Combo","DB.Combo","DB.Combo","DB.Combo","DB.Combo","DB.Combo","QU.Combo","QU.Combo","QU.Combo","QU.Combo","QU.Combo","QU.Combo","PTC.Combo","PTC.Combo","PTC.Combo","PTC.Combo","PTC.Combo","PTC.Combo","SNOT-22","SNOT-22","SNOT-22","SNOT-22","SNOT-22","SNOT-22")
...
d.AllwNP1<-na.omit(d.AllwNP[c("tissueid","PTC", "DB", "QUI", "SUC", "NaCl", "water", "DB_combo_prop", "QUI_combo_prop", "PTC_combo_prop", "snot22","control", "age", "sex", "TAS2R38", "EverSmoker", "asthma")])
...
for(var in varlist){
  ...
  fdendf.w <- summaryw$fstatistic[[3]]
  results <- data.frame(tidy(fit),fdendf.w)
  beta<-data.frame(tidy(lm(fmla, d.AllwNP))[2:6,2:5])
  beta[nrow(beta)+1,]<-NA
  results<-bind_cols(results, beta)
  results<-bind_cols(results, eta_squared(fit))
  ...
  leastsquarea = lsmeans(fit,
                      pairwise ~ control,
                      adjust="tukey", data=d.AllwNP)
  phc<-tidy(leastsquarea$contrasts)
  results_lm_wNPphc<-rbind(results_lm_wNPphc, phc)
}

results_lm_wNP$Compound<-c("PTC","PTC","PTC","PTC","PTC","PTC", "DB","DB","DB","DB","DB","DB", "Quinine","Quinine","Quinine","Quinine","Quinine","Quinine", "Sucrose","Sucrose","Sucrose","Sucrose","Sucrose","Sucrose","NaCl", "NaCl","NaCl","NaCl","NaCl","NaCl","H2O", "H2O","H2O","H2O","H2O","H2O","DB.Combo","DB.Combo","DB.Combo","DB.Combo","DB.Combo","DB.Combo", "QU.Combo","QU.Combo","QU.Combo","QU.Combo","QU.Combo","QU.Combo","PTC.Combo","PTC.Combo","PTC.Combo","PTC.Combo","PTC.Combo","PTC.Combo","SNOT-22","SNOT-22","SNOT-22","SNOT-22","SNOT-22","SNOT-22")
...
```{r, echo=FALSE, warning=FALSE}
results_lm_sNP<-results_lm_sNP[,c(14, 1:11,13)]
results_lm_sNP %>%
   format_model_table %>%
  kable (digits=1,caption="GLM results:CRSsNP vs Controls", col.names=c("Compound","term","df","sumsq","meansq","F","p","dendf","beta", "std.err", "statistic",  "p.value","Eta_Sq")) %>%
  kable_styling(bootstrap_options = c("hover", "condensed"), full_width = F)%>%
  row_spec(c(7,13,19,25, 31), bold = T, color = "white", background = "#D7261E") %>%
  row_spec(c(1,26), bold = T, color = "grey", background = "#E0E0E0")
```
...
```{r, echo=FALSE, warning=FALSE}
results_lm_wNP<-results_lm_wNP[,c(14,1:11,13)]
results_lm_wNP %>%
   format_model_table %>%
  kable (digits=1, col.names=c("Compound","term","df","sumsq","meansq","F","p","dendf", "beta", "std.err","statistic",  "p.value", "Eta_Sq")) %>%
  kable_styling(bootstrap_options = c("hover", "condensed"), full_width = F)%>%
  row_spec(c(7,13,19,25,31), bold = T, color = "white", background = "#D7261E") %>%
  row_spec(c(1,26), bold = T, color = "grey", background = "#E0E0E0")
```
...
In order to assess for differences in PTC taste scores we need to stratify our model by the three most common diplotypes - AVI/AVI, AVI/PAV, and PAV/PAV. First, we'll look at the classic linear model adjusting for age, sex, smoking and asmtha. 
### GLM with Covariate Analysis

```{r covariate_tas2r38, echo=FALSE, include=FALSE}
#AVI/AVI
mydata_avi<-mydata[which(mydata$Haplotype=="AVI/AVI"),]
avi_lm<-tidy(aov(lm(PTC ~ control+age+sex+EverSmoker+asthma, mydata_avi)))
beta<-data.frame(tidy(lm(PTC ~ control+age+sex+EverSmoker+asthma, mydata_avi))[2:6,2:5])
beta[nrow(beta)+1,]<-NA
avi_lm<-bind_cols(avi_lm, beta)
avi_lm<-bind_cols(avi_lm, eta_squared(aov(lm(PTC ~ control+age+sex+EverSmoker+asthma, mydata_avi))))
avi_lm2<-tidy(aov(lm(snot22 ~ control+age+sex+EverSmoker+asthma, mydata_avi)))

#AVI/PAV +PAV/PAV
mydata_pav<-mydata[which(mydata$Haplotype=="PAV/PAV" |mydata$Haplotype=="AVI/PAV" ),]
pav_lm<-tidy(aov(lm(PTC ~ control+age+sex+EverSmoker+asthma, mydata_pav)))
beta<-data.frame(tidy(lm(PTC ~ control+age+sex+EverSmoker+asthma, mydata_pav))[2:6,2:5])
beta[nrow(beta)+1,]<-NA
pav_lm<-bind_cols(pav_lm, beta)
pav_lm<-bind_cols(pav_lm, eta_squared(aov(lm(PTC ~ control+age+sex+EverSmoker+asthma, mydata_pav))))

pav_lm2<-tidy(aov(lm(snot22 ~ control+age+sex+EverSmoker+asthma, mydata_pav)))

lsmeans = lsmeans::lsmeans  ### Uses the lsmeans function
                            ###  from the lsmeans package,
                            ###  not from the lmerTest 

leastsquarea = lsmeans(aov(lm(PTC ~ control+age+sex+EverSmoker+asthma, mydata_pav)),
                      pairwise ~ control,
                      adjust="tukey", data=mydata_pav)
...
adj_stratified$Haplotype<-c("PAV-", "PAV-","PAV-","PAV-","PAV-","PAV-", "PAV+", "PAV+","PAV+","PAV+","PAV+","PAV+")
adj_stratified2$Haplotype<-c("PAV-", "PAV-","PAV-","PAV-","PAV-", "PAV-", "PAV+", "PAV+","PAV+","PAV+","PAV+","PAV+")
```

###CRS vs Controls
```{r, echo=FALSE}
adj_stratified<-adj_stratified[c(13,1:10,12)]
  kable (adj_stratified, caption="GLM CRS+Haplotype, PTC") %>%
  kable_styling(bootstrap_options = c("hover", "condensed"), full_width = F)%>%
  row_spec(c(1,7), bold = T, color = "white", background = "#D7261E")
```
...
### CRS vs Controls (3 levels)
```{r covariate_tas2r38+3levels, echo=FALSE, include=FALSE}
#AVI/AVI
mydata_avi<-mydata[which(mydata$Haplotype=="AVI/AVI"),]%>%
  mutate(control=ifelse(control=="TRUE", "CNTR", ifelse(control=="FALSE"& polyps==0, "CRSsNP", "CRSwNP")))
avi_lm<-tidy(aov(lm(PTC ~ control+age+sex+EverSmoker+asthma, mydata_avi)))
beta<-data.frame(tidy(lm(PTC ~ control+age+sex+EverSmoker+asthma, mydata_avi))[2:6,2:5])
beta[nrow(beta)+1,]<-NA
avi_lm<-bind_cols(avi_lm, beta)
...
```{r, echo=FALSE}
adj_stratified<-adj_stratified[c(13,1:10,12)]
  kable (adj_stratified, caption="GLM CRS+Haplotype+3levels, PTC") %>%
  kable_styling(bootstrap_options = c("hover", "condensed"), full_width = F)%>%
  row_spec(c(1,7), bold = T, color = "white", background = "#D7261E")
```
```{r, echo=FALSE}
  kable (PTC_pavphc, caption="GLM CRS+3levels pav+ _posthoc, PTC") %>%
  kable_styling(bootstrap_options = c("hover", "condensed"), full_width = F)
```
...
avi_lm<-tidy(aov(lm(PTC ~ control+age+sex+EverSmoker+asthma, d.AllsNP_avi)))
beta<-data.frame(tidy(lm(PTC ~ control+age+sex+EverSmoker+asthma, d.AllsNP_avi))[2:6,2:5])
beta[nrow(beta)+1,]<-NA
avi_lm<-bind_cols(avi_lm, beta)
avi_lm<-bind_cols(avi_lm, eta_squared(aov(lm(PTC ~ control+age+sex+EverSmoker+asthma, d.AllsNP_avi))))

#AVI/PAV, PAV/PAV
d.AllsNP_pav<-d.AllsNP[which(d.AllsNP$TAS2R38=="PAV/PAV" |d.AllsNP$TAS2R38=="AVI/PAV"),]
pav_lm<-tidy(aov(lm(PTC ~ control+age+sex+EverSmoker+asthma, d.AllsNP_pav)))
beta<-data.frame(tidy(lm(PTC ~ control+age+sex+EverSmoker+asthma, d.AllsNP_pav))[2:6,2:5])
beta[nrow(beta)+1,]<-NA
pav_lm<-bind_cols(pav_lm, beta)
pav_lm<-bind_cols(pav_lm, eta_squared(aov(lm(PTC ~ control+age+sex+EverSmoker+asthma, d.AllsNP_pav))))

lsmeans = lsmeans::lsmeans  ### Uses the lsmeans function
                            ###  from the lsmeans package,
                            ###  not from the lmerTest 

leastsquarea = lsmeans(aov(lm(PTC ~ control+age+sex+EverSmoker+asthma, d.AllsNP_pav)),
                      pairwise ~ control,
...
CRSsNPadj_stratified$Haplotype<-c("PAV-", "PAV-","PAV-","PAV-","PAV-", "PAV-", "PAV+","PAV+", "PAV+","PAV+","PAV+","PAV+")
```


```{r, echo=FALSE}
CRSsNPadj_stratified<-CRSsNPadj_stratified[,c(13,1:10,12)]
...
avi_lm<-tidy(aov(lm(PTC ~ control+age+sex+EverSmoker+asthma, d.AllwNP_avi)))
beta<-data.frame(tidy(lm(PTC ~ control+age+sex+EverSmoker+asthma, d.AllwNP_avi))[2:6,2:5])
beta[nrow(beta)+1,]<-NA
avi_lm<-bind_cols(avi_lm, beta)
avi_lm<-bind_cols(avi_lm, eta_squared(aov(lm(PTC ~ control+age+sex+EverSmoker+asthma, d.AllwNP_avi))))
#AVI/PAV, PAV/PAV
d.AllwNP_pav<-d.AllwNP[which(d.AllwNP$TAS2R38=="PAV/PAV" |d.AllwNP$TAS2R38=="AVI/PAV"),]

pav_lm<-tidy(aov(lm(PTC ~ control+age+sex+EverSmoker+asthma, d.AllwNP_pav)))
beta<-data.frame(tidy(lm(PTC ~ control+age+sex+EverSmoker+asthma, d.AllwNP_pav))[2:6,2:5])
beta[nrow(beta)+1,]<-NA
pav_lm<-bind_cols(pav_lm, beta)
pav_lm<-bind_cols(pav_lm, eta_squared(aov(lm(PTC ~ control+age+sex+EverSmoker+asthma, d.AllwNP_pav))))

lsmeans = lsmeans::lsmeans  ### Uses the lsmeans function
                            ###  from the lsmeans package,
                            ###  not from the lmerTest 

leastsquarea = lsmeans(aov(lm(PTC ~ control+age+sex+EverSmoker+asthma, d.AllwNP_pav)),
                      pairwise ~ control,
                      adjust="tukey", data=d.AllwNP_pav)
...
CRSwNPadj_stratified$Haplotype<-c("PAV-", "PAV-","PAV-","PAV-","PAV-","PAV-", "PAV+", "PAV+", "PAV+","PAV+","PAV+","PAV+")

```


```{r, echo=FALSE}
CRSwNPadj_stratified<-CRSwNPadj_stratified[,c(13,1:10,12)]
...
mydata1<-na.omit(mydatadf[c("tissueid","PTC", "DB", "QUI", "SUC", "NaCl", "water", "DB_combo_prop", "QUI_combo_prop", "PTC_combo_prop", "snot22","control", "age", "sex", "TAS2R38", "EverSmoker", "asthma")])
mydata1controls <-subset(mydata1,control=="TRUE")
mydata1cases <-subset(mydata1,control=="FALSE")

mydata1controls$PTC_pred<-predict(lm(PTC~age+sex+EverSmoker+asthma, data=mydata1controls))
mydata1controls$DB_pred<-predict(lm(DB~age+sex+EverSmoker+asthma, data=mydata1controls))
mydata1controls$QUI_pred<-predict(lm(QUI~age+sex+EverSmoker+asthma, data=mydata1controls))
mydata1controls$SUC_pred<-predict(lm(SUC~age+sex+EverSmoker+asthma, data=mydata1controls))
mydata1controls$NaCl_pred<-predict(lm(NaCl~age+sex+EverSmoker+asthma, data=mydata1controls))
mydata1controls$water_pred<-predict(lm(water~age+sex+EverSmoker+asthma, data=mydata1controls))

mydata1cases$PTC_pred<-predict(lm(PTC~age+sex+EverSmoker+asthma, data=mydata1cases))
mydata1cases$DB_pred<-predict(lm(DB~age+sex+EverSmoker+asthma, data=mydata1cases))
mydata1cases$QUI_pred<-predict(lm(QUI~age+sex+EverSmoker+asthma, data=mydata1cases))
mydata1cases$SUC_pred<-predict(lm(SUC~age+sex+EverSmoker+asthma, data=mydata1cases))
mydata1cases$NaCl_pred<-predict(lm(NaCl~age+sex+EverSmoker+asthma, data=mydata1cases))
mydata1cases$water_pred<-predict(lm(water~age+sex+EverSmoker+asthma, data=mydata1cases))

mydata1_results<-rbind(mydata1controls, mydata1cases)

forgraph_sample <- mydata1_results %>% 
  as_data_frame() %>%
...
forgraph_sample <- forgraph_sample %>% mutate(control = dplyr::recode(control, "1" = "Control", "0" = "Case")) %>% mutate(variable = dplyr::recode(variable, "DB_pred"="Denatonium Benzoate", "QUI_pred"="Quinine", "SUC_pred"="Sucrose", "NaCl_pred"="NaCl"))
 ...
 box_sample <- ggplot(forgraph_sample, aes(control, value)) +
  ...
  xlab("") + ylab("Taste Rating Scores") + theme(panel.background = element_rect(fill = 'white', color = 'white'), axis.line=element_line(size = 0.5, color = "black"), strip.background = element_rect(fill ="white", color = "white"), strip.text.x = element_text(size = 12, face = "bold"), axis.title.y = element_text(size = 12, face = "bold"))


 text_sample <- data.frame(
   label = c("p<0.001", "p=0.002", "p=0.014", "p=0.649"),
   variable   = c("Denatonium Benzoate", "Quinine", "Sucrose", "NaCl"),
   x     = c(1.5,1.5,1.5,1.5),
   y     = c(10,3.8,7.47,7.47)
)
p1_box_sample <- 
  box_sample +
...
###
library(dplyr)
forgraph_sample <- forgraph_sample %>% mutate(control = dplyr::recode(control, "Control" = "Control", "Case" = "CRS")) %>% mutate(variable = dplyr::recode(variable, "DB_pred"="Denatonium Benzoate", "QUI_pred"="Quinine", "SUC_pred"="Sucrose", "NaCl_pred"="NaCl"))

 box_sample <- ggplot(forgraph_sample, aes(control, value)) +
  geom_boxplot(outlier.colour="black", outlier.alpha =0.5,outlier.size = 0.2, notch=FALSE) + facet_wrap(~variable, ncol=1, scales = "free") +
  xlab("") + ylab("Taste Rating Scores") + theme(panel.background = element_rect(fill = 'white', color = 'white'), axis.line=element_line(size = 0.5, color = "black"), strip.background = element_rect(fill ="white", color = "white"), strip.text.x = element_text(size = 12, face = "bold"), axis.title.y = element_text(size = 12, face = "bold"))

text_sample2 <- data.frame(
   label = c("*", "*", "*", " "),
   variable   = c("Denatonium Benzoate", "Quinine", "Sucrose", "NaCl"),
   x     = c(2,2,2,2),
   y     = c(8.9,3.3,6.8,7)
...
p0_box_sample <- 
  ...
  geom_text(data  = text_sample, mapping = aes(x = x, y = y, label = label))+
  scale_y_continuous(breaks = my_breaks,limits = my_limits)+
  geom_text(data  = text_sample2, mapping = aes(x = x, y = y, label = label), size=8)


### Taste Scores of CRSwNP v. Controls  (Plotting Predicted Values)
d.AllwNP<-na.omit(d.AllwNP[c("tissueid","PTC", "DB", "QUI", "SUC", "NaCl", "water", "DB_combo_prop", "QUI_combo_prop", "PTC_combo_prop", "snot22","control", "age", "sex", "TAS2R38", "EverSmoker", "asthma")])
d.AllwNP_control <-subset(d.AllwNP,control=="TRUE")
d.AllwNP_cases <-subset(d.AllwNP,control=="FALSE")

d.AllwNP_control$PTC_pred<-predict(lm(PTC~age+sex+EverSmoker+asthma, data=d.AllwNP_control))
d.AllwNP_control$DB_pred<-predict(lm(DB~age+sex+EverSmoker+asthma, data=d.AllwNP_control))
d.AllwNP_control$QUI_pred<-predict(lm(QUI~age+sex+EverSmoker+asthma, data=d.AllwNP_control))
d.AllwNP_control$SUC_pred<-predict(lm(SUC~age+sex+EverSmoker+asthma, data=d.AllwNP_control))
d.AllwNP_control$NaCl_pred<-predict(lm(NaCl~age+sex+EverSmoker+asthma, data=d.AllwNP_control))
d.AllwNP_control$water_pred<-predict(lm(water~age+sex+EverSmoker+asthma, data=d.AllwNP_control))

d.AllwNP_cases$PTC_pred<-predict(lm(PTC~age+sex+EverSmoker+asthma, data=d.AllwNP_cases))
d.AllwNP_cases$DB_pred<-predict(lm(DB~age+sex+EverSmoker+asthma, data=d.AllwNP_cases))
d.AllwNP_cases$QUI_pred<-predict(lm(QUI~age+sex+EverSmoker+asthma, data=d.AllwNP_cases))
d.AllwNP_cases$SUC_pred<-predict(lm(SUC~age+sex+EverSmoker+asthma, data=d.AllwNP_cases))
d.AllwNP_cases$NaCl_pred<-predict(lm(NaCl~age+sex+EverSmoker+asthma, data=d.AllwNP_cases))
d.AllwNP_cases$water_pred<-predict(lm(water~age+sex+EverSmoker+asthma, data=d.AllwNP_cases))
...
box_wNP <- ggplot(forgraph_wNP, aes(control, value)) +
  ...
  theme(panel.background = element_rect(fill = 'white', color = 'white'), axis.line=element_line(size = 0.5, color = "black"), strip.background = element_rect(fill ="white", color = "white"), strip.text.x = element_text(size = 12, face = "bold"), axis.title.y = element_text(size = 12, face = "bold"))

text_wNP <- data.frame(
  label = c("p<0.001", "p<0.001", "p=0.006", "p=0.642"),
  variable   = c("Denatonium Benzoate", "Quinine", "Sucrose", "NaCl"),
  x     = c(1.5,1.5,1.5,1.5),
  y     = c(10,3.8,7.47,7.47)
)
...
d.AllsNP<-na.omit(d.AllsNP[c("tissueid","PTC", "DB", "QUI", "SUC", "NaCl", "water", "DB_combo_prop", "QUI_combo_prop", "PTC_combo_prop", "snot22","control", "age", "sex", "TAS2R38", "EverSmoker", "asthma")])
d.AllsNP_control <-subset(d.AllsNP,control=="TRUE")
d.AllsNP_cases <-subset(d.AllsNP,control=="FALSE")

d.AllsNP_control$PTC_pred<-predict(lm(PTC~age+sex+EverSmoker+asthma, data=d.AllsNP_control))
d.AllsNP_control$DB_pred<-predict(lm(DB~age+sex+EverSmoker+asthma, data=d.AllsNP_control))
d.AllsNP_control$QUI_pred<-predict(lm(QUI~age+sex+EverSmoker+asthma, data=d.AllsNP_control))
d.AllsNP_control$SUC_pred<-predict(lm(SUC~age+sex+EverSmoker+asthma, data=d.AllsNP_control))
d.AllsNP_control$NaCl_pred<-predict(lm(NaCl~age+sex+EverSmoker+asthma, data=d.AllsNP_control))
d.AllsNP_control$water_pred<-predict(lm(water~age+sex+EverSmoker+asthma, data=d.AllsNP_control))

d.AllsNP_cases$PTC_pred<-predict(lm(PTC~age+sex+EverSmoker+asthma, data=d.AllsNP_cases))
d.AllsNP_cases$DB_pred<-predict(lm(DB~age+sex+EverSmoker+asthma, data=d.AllsNP_cases))
d.AllsNP_cases$QUI_pred<-predict(lm(QUI~age+sex+EverSmoker+asthma, data=d.AllsNP_cases))
d.AllsNP_cases$SUC_pred<-predict(lm(SUC~age+sex+EverSmoker+asthma, data=d.AllsNP_cases))
d.AllsNP_cases$NaCl_pred<-predict(lm(NaCl~age+sex+EverSmoker+asthma, data=d.AllsNP_cases))
d.AllsNP_cases$water_pred<-predict(lm(water~age+sex+EverSmoker+asthma, data=d.AllsNP_cases))
...
text_sNP <- data.frame(
  label = c("p<0.001", "p=0.250", "p=0.231", "p=0.067"),
  variable   = c("Denatonium Benzoate", "Quinine", "Sucrose", "NaCl"),
  x     = c(1.5, 1.5,1.5,1.5),
  y     = c(10,3.8,7.47,7.47)
)
...
mydata1<-na.omit(mydatadf[c("tissueid","PTC", "DB", "QUI", "SUC", "NaCl", "water", "DB_combo_prop", "QUI_combo_prop", "PTC_combo_prop", "snot22","control", "age", "sex", "TAS2R38", "EverSmoker", "asthma", "polyps")])

mydata1$control=ifelse(mydata1$control=="TRUE", "CNTR", ifelse(mydata1$control=="FALSE" & mydata1$polyps==0, "CRSsNP", "CRSwNP"))
  
mydata1controls <-subset(mydata1,control=="CNTR")
mydata1CRSsNP <-subset(mydata1,control=="CRSsNP")
mydata1CRSwNP <-subset(mydata1,control=="CRSwNP")
...
mydata1CRSsNP$PTC_pred<-predict(lm(PTC~age+sex+EverSmoker+asthma, data=mydata1CRSsNP))
mydata1CRSsNP$DB_pred<-predict(lm(DB~age+sex+EverSmoker+asthma, data=mydata1CRSsNP))
mydata1CRSsNP$QUI_pred<-predict(lm(QUI~age+sex+EverSmoker+asthma, data=mydata1CRSsNP))
mydata1CRSsNP$SUC_pred<-predict(lm(SUC~age+sex+EverSmoker+asthma, data=mydata1CRSsNP))
mydata1CRSsNP$NaCl_pred<-predict(lm(NaCl~age+sex+EverSmoker+asthma, data=mydata1CRSsNP))
mydata1CRSsNP$water_pred<-predict(lm(water~age+sex+EverSmoker+asthma, data=mydata1CRSsNP))

mydata1CRSwNP$PTC_pred<-predict(lm(PTC~age+sex+EverSmoker+asthma, data=mydata1CRSwNP))
mydata1CRSwNP$DB_pred<-predict(lm(DB~age+sex+EverSmoker+asthma, data=mydata1CRSwNP))
mydata1CRSwNP$QUI_pred<-predict(lm(QUI~age+sex+EverSmoker+asthma, data=mydata1CRSwNP))
mydata1CRSwNP$SUC_pred<-predict(lm(SUC~age+sex+EverSmoker+asthma, data=mydata1CRSwNP))
mydata1CRSwNP$NaCl_pred<-predict(lm(NaCl~age+sex+EverSmoker+asthma, data=mydata1CRSwNP))
mydata1CRSwNP$water_pred<-predict(lm(water~age+sex+EverSmoker+asthma, data=mydata1CRSwNP))

mydata1_results<-rbind(mydata1controls, mydata1CRSsNP,mydata1CRSwNP)
...
library(dplyr)
forgraph_sample <- forgraph_sample %>% mutate(control = dplyr::recode(control, "CNTR" = "Control", "CRSsNP" = "CRSsNP", "CRSwNP"="CRSwNP")) %>% mutate(variable = dplyr::recode(variable, "DB_pred"="Denatonium Benzoate", "QUI_pred"="Quinine", "SUC_pred"="Sucrose", "NaCl_pred"="NaCl"))
...
box_sample <- ggplot(forgraph_sample, aes(control, value)) +
 ...
 text_sample <- data.frame(
   label = c("p<0.001", "p=0.001", "p=0.023", "p=0.065"),
   variable   = c("Denatonium Benzoate", "Quinine", "Sucrose", "NaCl"),
   x     = c(2,2,2,2),
   y     = c(10,3.8,7.47,7.47)
)
 ...
 text_sample2 <- data.frame(
   ...
   variable   = c("Denatonium Benzoate", "Quinine", "Sucrose", "NaCl"),
   x     = c(3,3,3,3),
   y     = c(8.8,3.8,7.3,7.47)
)
 
 text_sample3 <- data.frame(
   label = c("*", " ", " ", " "),
   variable   = c("Denatonium Benzoate", "Quinine", "Sucrose", "NaCl"),
   x     = c(2,2,2,2),
   y     = c(8.9,3.8,7.3,7.47)
)
...
p4_box_sample <- 
  ...
  scale_y_continuous(breaks = my_breaks,limits = my_limits)+
  geom_text(data  = text_sample2, mapping = aes(x = x, y = y, label = label), size=8)+
  geom_text(data  = text_sample3, mapping = aes(x = x, y = y, label = label), size=8)
...
ggsave(filename ='Figure_1_v4.tif', box_crs,  width = 20, height = 28, units ="cm",dpi = 1200, device='tiff', limitsize = TRUE, compression = "lzw")
dev.off()

```
### Boxplots of Taste Scores of CRs+CRSwNP+CRSsNp v. Controls  (Plotting Predicted Values(option1))
```{r, echo=FALSE, fig.width =12, fig.height = 18}
box_crs<-ggarrange(p0_box_sample, p4_box_sample, ncol=2, labels=c("A", "B"), 
widths=c(1, 1.4)) 
box_crs

postscript(colormodel="cmyk")
ggsave(filename ='Figure_1.tif', box_crs,  width = 17, height = 28, units ="cm",dpi = 1200, device='tiff', limitsize = TRUE, compression = "lzw")
dev.off()
...
#CRS
m.data_gen<-mydata1%>%
  select(tissueid, control, PTC,age,sex,EverSmoker,asthma,TAS2R38)%>%
  drop_na()%>%
  mutate(control=ifelse(control=="CNTR", "TRUE", "FALSE"))
...
m.data_gen_neg1$PTC1 <- predict(lm(PTC ~age + sex + EverSmoker + asthma, subset(m.data_gen_neg, control=="TRUE")))
m.data_gen_neg2$PTC1<-predict(lm(PTC ~ age + sex + EverSmoker+asthma,subset(m.data_gen_neg,control=="FALSE")))
...
m.data_gen_pos1$PTC1 <- predict(lm(PTC ~  age + sex + EverSmoker+asthma,subset(m.data_gen_pos, control=="TRUE")))
m.data_gen_pos2$PTC1 <- predict(lm(PTC ~ age + sex + EverSmoker+asthma,subset(m.data_gen_pos, control=="FALSE")))
...
#CRSwNP
#matched_wnP_gen <- mydata(control ~ age + sex + EverSmoker, data = d.AllsNP, method= "genetic")
...
m.data_allwNP_gen_neg1$PTC1 <- predict(lm(PTC ~ age + sex + EverSmoker+asthma,subset(m.data_allwNP_gen_neg, control=="TRUE")))
m.data_allwNP_gen_neg2$PTC1 <- predict(lm(PTC ~  age + sex + EverSmoker+asthma,subset(m.data_allwNP_gen_neg, control=="FALSE")))
...
m.data_allwNP_gen_pos1$PTC1 <- predict(lm(PTC ~ age + sex + EverSmoker+asthma,subset(m.data_allwNP_gen_pos, control=="TRUE")))
m.data_allwNP_gen_pos2$PTC1 <- predict(lm(PTC ~ age + sex + EverSmoker+asthma,subset(m.data_allwNP_gen_pos, control=="FALSE")))
...
#CRSsNP
#matched_sNP_gen <- mydata(control ~ age + sex + EverSmoker, data = d.AllsNP, method= "genetic")
...
m.data_allsNP_gen_neg1$PTC1 <- predict(lm(PTC ~ age + sex + EverSmoker+asthma,subset(m.data_allsNP_gen_neg, control=="TRUE")))
m.data_allsNP_gen_neg2$PTC1 <- predict(lm(PTC ~ age + sex + EverSmoker+asthma,subset(m.data_allsNP_gen_neg, control=="FALSE")))
...
m.data_allsNP_gen_pos1$PTC1 <- predict(lm(PTC ~  age + sex + EverSmoker+asthma,subset(m.data_allsNP_gen_pos, control=="TRUE")))
m.data_allsNP_gen_pos2$PTC1 <- predict(lm(PTC ~ age + sex + EverSmoker+asthma,subset(m.data_allsNP_gen_pos, control=="FALSE")))
...
m.data_gen_fin$control<-as.logical(m.data_gen_fin$control)
...
box_all <- ggplot(forgraph_all, aes(T2R38, PTC1)) + 
      geom_boxplot(aes(color=control), outlier.color = "black", outlier.size = 0.2, outlier.alpha = 0.5, lwd=0.6) +
      facet_wrap(~dat) + 
      xlab("TAS2R38 diplotype")+
      ylab("PTC Taste Score") +
      ylim(0,10) + 
      theme(axis.text.x = element_text(angle=0, hjust = 0.5, vjust=1))+
      theme(panel.background =element_blank(), axis.line=element_line(size = 0.5, color = "black"), strip.background = element_blank(), strip.text.x = element_text(size = 12, face = "bold"), legend.position = c(0.065, 0.95), legend.background = element_blank(), legend.key=element_blank()) + scale_color_manual(name="", values=c("black", "grey"))

text_forgraph <- data.frame(
  label = c( "p=0.787","p<0.001", "p=0.278", "p=0.034", "p=0.628", "p=0.001"),
  dat   = c("CRS", "CRS", "CRSsNP", "CRSsNP", "CRSwNP", "CRSwNP"),
  x     = c(1,2,1,2,1,2),
  y     = c(2,9,2,9,2,9)
)

text_forgraph2 <- data.frame(
  label = c( "","*", "", "", "", ""),
  dat   = c("CRS", "CRS", "CRSsNP", "CRSsNP", "CRSwNP", "CRSwNP"),
  x     = c(1,1.81,1,2,1,2),
  y     = c(2,8.1,2,9,2,9)
...
final_box_all <- 
  ...
  theme(plot.title = element_text(hjust = 0.5))+geom_text(data  = text_forgraph2, mapping = aes(x = x, y = y, label = label), size=6)
...
postscript(colormodel="cmyk")
ggsave(filename ='Figure_2_v4.tif', final_box_all,  width = 20, height = 8, units ="cm",dpi = 1200, device='tiff', limitsize = TRUE, compression = "lzw")
dev.off()
```


```{r, echo=FALSE}
m.data_gen_fin<-m.data_gen_fin%>%
  filter(T2R38 != "Other")
m.data_gen_fin$control<-as.character(m.data_gen_fin$control)
m.data_gen_fin<- m.data_gen_fin %>% mutate(control = dplyr::recode(control, "TRUE" = "Control", "FALSE" = "CRS"))

box_all <- ggplot(m.data_gen_fin, aes(T2R38, PTC1)) + 
      geom_boxplot(aes(color=control), outlier.color = "black", outlier.size = 0.2, outlier.alpha = 0.5, lwd=0.6) +
      xlab("TAS2R38 diplotype")+
      ylab("PTC Taste Score") +
      ylim(0,10) + 
      theme(axis.text.x = element_text(angle=0, hjust = 0.5, vjust=1))+
      theme(panel.background =element_blank(), axis.line=element_line(size = 0.5, color = "black"), strip.background = element_blank(), strip.text.x = element_text(size = 12, face = "bold"), legend.position = c(0.2, 0.95), legend.background = element_blank(), legend.key=element_blank()) + scale_color_manual(name="", values=c("grey", "black"))

text_forgraph <- data.frame(
  label = c( "p=0.787","p<0.001"),
  dat   = c("CRS", "CRS"),
  x     = c(1,2),
  y     = c(2,9)
)
...
### Fig2B
forgraph_all <- bind_rows(m.data_allwNP_gen_fin, m.data_allsNP_gen_fin)%>%
  filter(T2R38 != "Other")
forgraph_all$control <- as.numeric(forgraph_all$control)
forgraph_all<- forgraph_all %>% mutate(control = dplyr::recode(control, "1" = "Control", "0" = "Case"))%>%
  mutate(control=ifelse(control=="Case" & dat=="CRSsNP", "CRSsNP", ifelse(
   control=="Case" & dat=="CRSwNP", "CRSwNP", "Control")))

box_all2 <- ggplot(forgraph_all, aes(T2R38, PTC1)) + 
      geom_boxplot(aes(color=control), outlier.color = "black", outlier.size = 0.2, outlier.alpha = 0.5, lwd=0.6) +
      xlab("TAS2R38 diplotype")+
      ylab("PTC Taste Score") +
      ylim(0,10) + 
      theme(axis.text.x = element_text(angle=0, hjust = 0.5, vjust=1))+
      theme(panel.background =element_blank(), axis.line=element_line(size = 0.5, color = "black"), strip.background = element_blank(), strip.text.x = element_text(size = 12, face = "bold"), legend.position = c(0.2, 0.9), legend.background = element_blank(), legend.key=element_blank()) + scale_color_manual(name="", values=c("grey","dimgrey","black"))

text_forgraph3 <- data.frame(
  label = c( "p=0.231","p=0.003"),
  dat   = c( "CRSsNP", "CRSwNP"),
  x     = c(1,2),
  y     = c(2,9)
)
...
final_box_all2 <- 
  box_all2 + 
  geom_text(data  = text_forgraph3, mapping = aes(x = x, y = y, label = label)) +
  theme(plot.title = element_text(hjust = 0.5))

```

### Boxplots of Taste Scores of CRs+CRSwNP+CRSsNp v. Controls  (Plotting Predicted Values(option2))
```{r, echo=FALSE, fig.width =12, fig.height = 7}
box_tas2r38<-ggarrange(final_box_all, final_box_all2, ncol=2, labels=c("A", "B"),
widths=c(1, 1.4)) 
box_tas2r38

postscript(colormodel="cmyk")
ggsave(filename ='Figure_2.tif', box_tas2r38,  width = 17, height = 8, units ="cm",dpi = 1200, device='tiff', limitsize = TRUE, compression = "lzw")
dev.off()
