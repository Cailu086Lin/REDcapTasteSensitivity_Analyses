...
mydirectory<-"C:/Users/clin/Dropbox/REDCap/Archive/Version 4/data/dat"
...
# Load  library
pacman::p_load(readxl, plyr, dplyr, mice, knitr, magrittr, tableone, reshape, kableExtra, broom, stringr, rgenoud, tidyverse, Hmisc, uwIntroStats, ggpubr, epitools, survey, lubridate, HardyWeinberg, stringr, data.table)
```
...
```
CRS=Chronic rhinosinusitis; P=p-value; SMD=standardized mean differences; N=number of subjects; %=percent; White = people of European ancestry by self-report, Non-white = otherwise; yrs=years of age; SNOT-22, Sinonasal Outcome Test, scores range from 0 to 110 with higher values indicating worse symptoms. TAS2R38 diplotype, the combination of two haplotypes of the bitter receptor, PAV is the â€˜tasterâ€? form whereas the AVI is the â€˜non-tasterâ€? form. 
...
```
Note: The â€œDB.Comboâ€? score was generated by taking the subjects DB proportion (DB score over total taste test score) and adding it to one minus their Sucrose proportion (Sucrose score over total taste test score). The â€œQU.Comboâ€? score was generated in the same fashion, except using the subjectâ€™s Quinine proportion instead of DB proportion.
...
mydata1<-mydatadf[c("tissueid","PTC", "DB", "QUI", "SUC", "NaCl", "water", "DB_combo_prop", "QUI_combo_prop", "PTC_combo_prop", "snot22","control", "age", "sex", "TAS2R38", "EverSmoker" )]
#loop
...
d.CRSsNP<-mydatadf[which(mydatadf$polyps=="0" & mydatadf$control==0),] #cases with polyps
...
results_lm_sNP %>%
   format_model_table %>%
  kable (digits=1,caption="GLM results:CRSsNP vs Controls", col.names=c("Compound","term","df","sumsq","meansq","F","p","dendf")) %>%
  kable_styling(bootstrap_options = c("hover", "condensed"), full_width = F)%>%
  row_spec(c(6,11,16,21), bold = T, color = "white", background = "#D7261E") %>%
  row_spec(c(1,26), bold = T, color = "grey", background = "#E0E0E0")
```
...
avi_lm<-tidy(aov(lm(PTC ~ control+age+sex+EverSmoker, mydata_avi)))
avi_lm2<-tidy(aov(lm(snot22 ~ control+age+sex+EverSmoker, mydata_avi)))

#AVI/PAV +PAV/PAV
mydata_pav<-mydata[which(mydata$Haplotype=="PAV/PAV" |mydata$Haplotype=="AVI/PAV" ),]
pav_lm<-tidy(aov(lm(PTC ~ control+age+sex+EverSmoker, mydata_pav)))
pav_lm2<-tidy(aov(lm(snot22 ~ control+age+sex+EverSmoker, mydata_pav)))

lsmeans = lsmeans::lsmeans  ### Uses the lsmeans function
                            ###  from the lsmeans package,
                            ###  not from the lmerTest 

leastsquarea = lsmeans(aov(lm(PTC ~ control+age+sex+EverSmoker, mydata_pav)),
                      pairwise ~ control,
                      adjust="fisher", data=mydata_pav)
...
mydata1<-na.omit(mydatadf[c("tissueid","PTC", "DB", "QUI", "SUC", "NaCl", "water", "DB_combo_prop", "QUI_combo_prop", "PTC_combo_prop", "snot22","control", "age", "sex", "TAS2R38", "EverSmoker" )])
mydata1controls <-subset(mydata1,control=="TRUE")
mydata1cases <-subset(mydata1,control=="FALSE")

mydata1controls$PTC_pred<-predict(lm(PTC~age+sex+EverSmoker, data=mydata1controls))
mydata1controls$DB_pred<-predict(lm(DB~age+sex+EverSmoker, data=mydata1controls))
mydata1controls$QUI_pred<-predict(lm(QUI~age+sex+EverSmoker, data=mydata1controls))
mydata1controls$SUC_pred<-predict(lm(SUC~age+sex+EverSmoker, data=mydata1controls))
mydata1controls$NaCl_pred<-predict(lm(NaCl~age+sex+EverSmoker, data=mydata1controls))
mydata1controls$water_pred<-predict(lm(water~age+sex+EverSmoker, data=mydata1controls))

mydata1cases$PTC_pred<-predict(lm(PTC~age+sex+EverSmoker, data=mydata1cases))
mydata1cases$DB_pred<-predict(lm(DB~age+sex+EverSmoker, data=mydata1cases))
mydata1cases$QUI_pred<-predict(lm(QUI~age+sex+EverSmoker, data=mydata1cases))
mydata1cases$SUC_pred<-predict(lm(SUC~age+sex+EverSmoker, data=mydata1cases))
mydata1cases$NaCl_pred<-predict(lm(NaCl~age+sex+EverSmoker, data=mydata1cases))
mydata1cases$water_pred<-predict(lm(water~age+sex+EverSmoker, data=mydata1cases))
...
box_all <- ggplot(forgraph_all, aes(T2R38, PTC1)) + 
      geom_boxplot(aes(color=control), outlier.color = "black", outlier.size = 0.2, outlier.alpha = 0.5) +
      facet_wrap(~dat) + 
      ...
      labs(color="") +
      theme(panel.background = element_rect(fill = 'white', color = 'white'), axis.line=element_line(size = 0.5, color = "black"), strip.background = element_rect(fill ="white", color = "white"), strip.text.x = element_text(size = 12, face = "bold"), legend.position = c(0.065, 0.95))+scale_color_manual(values=c("black","#999999"))
